...
func init() {
	...
	postsTable, err := Db.Prepare("CREATE TABLE if not exists POSTS(ID integer primary key, CATEGORIE text, CATEGORIE2 text, CATEGORIE3 text, CATEGORIE4 text, POST text, USERNAME text, LIKECOUNT integer default 0, DISLIKECOUNT integer default 0, COMMENTCOUNT integer default 0, IMG blob)")
	if err != nil {
		log.Fatal(err)
	}
	postsTable.Exec()
